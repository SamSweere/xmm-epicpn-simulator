...
# Create user
RUN useradd xmm_user --create-home --no-log-init
...
RUN ${HOME}/miniconda.sh -b -u -p ${MINICONDA} && rm ${HOME}/miniconda.sh
...
COPY --chown=xmm_user --chmod=777 downloads/heasoft-6.33.2/ $HOME/heasoft-6.33.2/

WORKDIR $HOME/heasoft-6.33.2/BUILD_DIR/
...
COPY --chown=xmm_user --chmod=777 downloads/sixte-3.0.2 $HOME/sixte_src/
...
COPY --from=heasoft --chown=xmm_user --chmod=777 ${HEADAS} ${HEADAS}
COPY --from=sas --chown=xmm_user --chmod=777 ${SAS_ROOT} ${SAS_ROOT}
COPY --from=sixte --chown=xmm_user --chmod=777 ${SIXTE} ${SIXTE}
