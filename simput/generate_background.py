...
def create_back_fits(run_dir):
    # Generate the background image used for the simput
    resolution = 402
    n = np.ones((resolution, resolution))
    ...
    header['CRPIX1'] = int(
        resolution / 2) - 44  # For some reason I need to offset it in the other direction#226 #n.shape[0] / 2  # 0.0
    ...
    out_file = os.path.join(run_dir, 'const_background.fits')
...
def generate_ascii_spectrum(run_dir, spectrum_file, verbose):
    ...
    out_file = os.path.join(run_dir, "background_spectrum.txt")
...
def generate_background_simput(run_dir, spectrum_file, emin, emax, verbose):
    # Create the background fits
    background_file = create_back_fits(run_dir)

    # Create the ascii spectrum file
    ascii_spectrum_file = generate_ascii_spectrum(run_dir, spectrum_file, verbose)

    simput_name = f"background.simput"
    outfile_path = os.path.join(run_dir, simput_name)
