...
def get_ccf_path() -> Path:
    ...
    if not ccf_path.exists():
        raise NotADirectoryError(
            f"The CCF directory does not exist at {ccf_path.resolve()}"
        )
    return ccf_path
...
def get_epn_lincoord() -> Path:
    ...
    if not epn_lincoord:
        raise FileExistsError(
            f"Could not find any EPN_LINCOORD*.CCF in '{ccf_path.resolve()}'!"
        )
    epn_lincoord.sort()
...
def get_emos_lincoord(emos_num: Literal[1, 2]):
    ...
    if not emos_lincoord:
        raise FileExistsError(
            f"Could not find any EMOS{emos_lincoord}_LINCOORD*.CCF in '{ccf_path.resolve()}'!"
        )
    emos_lincoord.sort()
...
def get_xmm_miscdata() -> Path:
    ...
    if not xmm_miscdata:
        raise FileExistsError(
            f"Could not find any XMM_MISCDATA*.CCF in '{ccf_path.resolve()}'!"
        )
    xmm_miscdata.sort()
...
def get_xrt_xareaef(instrument_name: Literal["epn", "emos1", "emos2"]):
    ...
    if not xrt_xareaef:
        raise FileExistsError(
            f"Could not find any {telescope}_XAREAEF*.CCF in '{ccf_path.resolve()}'!"
        )
    xrt_xareaef.sort()
